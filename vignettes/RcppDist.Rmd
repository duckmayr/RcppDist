...
The Laplace distribution is the distribution of differences between two independent exponential variates with identical distributions with scale $b$, shifted by the location parameter $\mu$. The functions provided in this header are:

```{Rcpp laplace, eval = FALSE}
double d_laplace(const double x, const double mu, const double b,
        const int log_p = 0)

double p_laplace(const double q, const double mu, const double b,
        const int lower_tail = 1, const int log_p = 0)

double q_laplace(const double p, const double mu, const double b,
        const int lower_tail = 1, const int log_p = 0)

double r_laplace(const double mu, const double b)

Rcpp::NumericVector dlaplace(const Rcpp::NumericVector& x, const double mu,
        const double b, const bool log_p = false)

Rcpp::NumericVector plaplace(const Rcpp::NumericVector& q, const double mu,
        const double b, const bool lower_tail = true, const bool log_p = false)

Rcpp::NumericVector qlaplace(const Rcpp::NumericVector& p, const double mu,
        const double b, const bool lower_tail = true, const bool log_p = false)

Rcpp::NumericVector rlaplace(const int n, const double mu, const double b,
...
arma::vec dmvnorm(const arma::mat& x, const arma::rowvec& mu,
        const arma::mat& S, const bool log_p = false)

arma::mat rmvnorm(const arma::uword n, const arma::vec& mu,
        const arma::mat& S)
        
arma::mat rmvnorm(const arma::uword n, const arma::rowvec& mu,
        const arma::mat& S)
```
...
Notice that the functions are overloaded such that you can pass `mu` either as an `arma::vec` or as an `arma::rowvec`.
...
arma::vec dmvt(const arma::mat& x, const arma::rowvec& mu,
        const arma::mat& S, const double df, const bool log_p = false)

arma::mat rmvt(const arma::uword n, const arma::vec& mu,
        const arma::mat& S, const double df)

arma::mat rmvt(const arma::uword n, const arma::rowvec& mu,
        const arma::mat& S, const double df)
```
